...
// ---- security headers -------------------------------------------------------
// the page only ever needs its own files, gifenc off jsDelivr, and blob: URLs
// for the video and the finished GIF. locking everything else down means a
// hostile video (or anything that sneaks past the proxy's type check) has
// nowhere to phone home to and no way to run script in our origin.
// style-src keeps 'unsafe-inline' because the CSS lives in index.html
const CSP = [
  "default-src 'self'",
